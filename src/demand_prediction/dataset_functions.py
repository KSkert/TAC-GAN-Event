...
def create_events_df(data, events, emb_only=False, emb_size=100):
    ...
    # Merge and drop rows without matching events (just like A)
    data = data.merge(events, on='date', how='left').dropna()

    # Keep B's specific columns
    data = data[['date', 'Category', 'embedding', 'wiki_name',
                 'High-Category', 'country', 'view_seq', 
                 'views_on_event_day', 'label', 'type', 'global_relevance_rank', 'cosine_sim']]

    # Expand embedding into separate columns
    ...
    if not emb_only:
        ...
        data = data.join(one_hot)
        one_hot = pd.get_dummies(data['cosine_sim'])
        data = data.join(one_hot)
        one_hot = pd.get_dummies(data['global_relevance_rank'])
        data = data.join(one_hot)
